...
export const useMydeleteDoctor = (doctorId: string) => {
  const naviagte = useNavigate()
  const { setCurrentUser } = useUser()
  const deleteDoctor = async () => {
    const responce = await fetch(
      `${BACKEND_API_URL}/api/doctor/delete/${doctorId as string}`,
      {
        method: "DELETE",
        credentials: "include",
      }
  ...
  const { data: doctorDelete, isLoading } = useQuery(
    "deleteDoctor",
    deleteDoctor,
    {
      onSuccess: () => {
        toast.success("Doctor successfuly deleted")
      },
      onError: () => {
